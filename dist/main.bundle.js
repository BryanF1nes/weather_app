...
async function fetchCurrentData(search = "London") {
  try {
    ...
    const response = await fetch(
      `${"http://api.weatherapi.com/v1/"}current.json?key=${key}&q=${search}&aqi=no`
    );
...
async function fetchForecastData(search = "London") {
  try {
    const key = "c549879f5d5e45b8b4433616231805";
    const response = await fetch(
      `${"http://api.weatherapi.com/v1/"}forecast.json?key=${key}&q=${search}&days=3&aqi=no&alerts=no`
    );
    const data = await response.json();
